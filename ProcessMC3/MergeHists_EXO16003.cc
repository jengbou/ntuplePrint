...
void MergeHists_EXO16003(float goalintlum,int nbin, float* xsec, int* nfiles, std::string* binnames,std::string aaname,std::string ohname,bool donorm, std::string bbname)
{
    ...
    if(donorm) {
        qcdtools->HistNorm(norm,nbin,xsec,nfiles,binnames,bbname);  // this gives the total number of events in each bin before all selections using the eventCountPreTrigger histogram
    } else{
...
//         "hjptfra2","hjptfrbc","hjptfra1c","hjptfra2c","hjptb",
    ...
    for(int i=0;i<nhist;i++) {
        ...
        vv[i]=qcdtools->HistMan(goalintlum,histnames[i],norm,outnorm,nbin,xsec,nfiles,binnames,donorm,bbname);
    }
    ...
    for(int i=0;i<nhist2;i++) {
        ...
        vv2[i]=qcdtools->HistMan2(goalintlum,histnames2[i],norm,outnorm2,nbin,xsec,nfiles,binnames,donorm,bbname);
    }
