  ...
  */
  ...
  for (Int_t i=0; i<nentries; i++) {
    ...
    for(Int_t j=0; j<NNNjet; j++) {
      ...
      if(((*jet_pt)[j]>40)&&(fabs((*jet_eta)[j])<2.4)) {
	HT=HT+(*jet_pt)[j];
      }
      ...
      for(int it=0;it<track_pts.size();it++) sort_ipsig[it]=0;
      jntrack[j]=0;
	...
	if((track_sources[itrack]==0)&&(track_pts[itrack]>1)) {
      ...
      if(jntrack[j]>0) jet_logmeanipsig[j]=jet_logmeanipsig[j]/atmp;
      ...
      std::sort(sort_ipsig.begin(), sort_ipsig.end());
      std::reverse(sort_ipsig.begin(),sort_ipsig.end());
	...
	if( (fabs((*jet_eta)[ij])<2.0)) { // jet  eta
	...
	if((*jet_nef)[ij]<0.9) {  // neutral fraction
	    ...
	    if(jet_ntrkpt1[ij]>0) {  // tracks pt>1
		...
		if((*jet_pt)[ij]>40) {
		  if(jntrkip1mm[ij]<3) {
		   ...
		   //now do emerging jets
		if((*jet_pt)[ij]>60) {  // jet pt

	        if(otfile) hbcut_alphamax->Fill((*jet_alphaMax)[ij]);
	        if((*jet_alphaMax)[ij]<0.04) { // alpha max
		...
		if(ij<4) {
		  ...
		  */
		if(jet_logmeanipsig[ij]>1.5) { // log ipsig
