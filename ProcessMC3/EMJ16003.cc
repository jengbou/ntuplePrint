...
//     tt->SetBranchAddress("HLT_HT350_DisplacedDijet40_DisplacedTrack",&hltTrig2n);
    ...
    */
    ...
    if(otfile) {
        ...
        h_nloosetag = new TH1F("h_nloosetag","number of loose tagged displaced jets",20,0.,20.);
        ...
        H_T1      = new TH1F("H_T1"," HT distribution after trigger jet trk requirements cut", 100,0.,5000.);
        H_T2      = new TH1F("H_T2"," HT distribution after HT cut", 100,0.,5000.);
        H_T3      = new TH1F("H_T3"," HT distribution after kinematic cut", 100,0.,5000.);
        H_T4      = new TH1F("H_T4"," HT distribution at end (SR)", 100,0.,5000.);
        ...
        hfr_ntrkpt1d = new TH1F("hfr_ntrkpt1d","number tracks pt>1 of jets in events after kinematic cut",20,0.,20.);
        hfr_ntrkpt1n = new TH1F("hfr_ntrkpt1n","number tracks pt>1 of jets passing disp. jet tagger",20,0.,20.);
        ...
        // HLT
        hTrig1d = new TH1F("hTrig1d","HLT_HT400_DispDijet40_Incl",300,0.,3000.);
        hTrig1n = new TH1F("hTrig1n","HLT_HT500_DispDijet40_Incl",300,0.,3000.);
        hTrig2d = new TH1F("hTrig2d","HLT_HT250_DispDijet40_DispTrack",300,0.,3000.);
        hTrig2n = new TH1F("hTrig2n","HLT_HT350_DispDijet40_DispTrack",300,0.,3000.);
        hTrig3d = new TH1F("hTrig3d","HLT_PFHT600",300,0.,3000.);
        hTrig3n = new TH1F("hTrig3n","HLT_PFHT900",300,0.,3000.);
    ...
    for (Int_t i=0; i<nentries; i++) {
        ...
        for(Int_t j=0; j<NNNjet; j++) {
            ...
            if (otfile) {
                halphavtheta2D->Fill((*jet_alphaMax)[j],jet_medtheta2D[j]);
                halphavipXYSig->Fill((*jet_alphaMax)[j],jet_medipsig[j]);
        ...
        for(int ij=0;ij<NNNjet;ij++) {
            ...
            if((fabs((*jet_eta)[ij])<2.0)) { // Trig/kinematic eta cut
                ...
                if( (*jet_pt)[ij]>60 ) {  // jet pt kinematic cut
                    ...
                    // Loose jet taggers for fake rate
                    if((*jet_alphaMax)[ij]<0.5) { // alpha max 50%
                        nloosetag1+=1;
                        if(jet_logmedipsig[ij]>0.434) { // log median ipsig
                            nloosetag2+=1;
                            if (jet_logmedtheta2D[ij]>-1.7) {// log median theta2D
                                nloosetag3+=1;
                            }
                        }
                    }
        ...
        if(otfile) h_nalemg->Fill(ntag1);
        if(otfile) h_nemg->Fill(ntag2);
        if(otfile) h_ntag->Fill(ntag3);
        if(otfile) h_nloosetag->Fill(nloosetag3);
        ...
        // Loose tag
        bool Cnltag1ge1 = false;
        if (nloosetag1>0) Cnltag1ge1=true;

        bool Cnltag1ge2 = false;
        if (nloosetag1>1) Cnltag1ge2=true;

        // number emerging jets
        bool Cnltag2ge1 = false;
        if(nloosetag2>0) Cnltag2ge1=true;

        bool Cnltag2ge2 = false;
        if(nloosetag2>1) Cnltag2ge2=true;

        // number tagged jets
        bool Cnltag3ge1 = false;
        if (nloosetag3>0) Cnltag3ge1=true;

        bool Cnltag3ge2 = false;
        if (nloosetag3>1) Cnltag3ge2=true;

        // Nominal
        bool Cnalem1 = false;
        if (ntag1>0) Cnalem1=true;

        bool Cnalem2 = false;
        if (ntag1>1) Cnalem2=true;

        // number emerging jets
        bool Cnem1 = false;
        if(ntag2>0) Cnem1=true;

        bool Cnem2 = false;
        if(ntag2>1) Cnem2=true;

        // number tagged jets
        bool Cntag1 = false;
        if (ntag3>0) Cntag1=true;

        bool Cntag2 = false;
        if (ntag3>1) Cntag2=true;
        ...
        if(Ctrig1||Ctrig2) {//this incl. HLT
            if(otfile) count->Fill("TrigJetTrk",1);
                ...
                if(Ckine) {
                    ...
                    for(int i=0;i<NNNjet;i++) {
                        if (otfile){
                            ...
                            hlogmedtheta2DPS->Fill(jet_logmedtheta2D[i]);
                            ...
                            // EXO-16-003 FR
                            if (ntag3<2) {
                                hfr_ntrkpt1d->Fill(jet_ntrkpt1[i]);
                                if (tagged[i]) hfr_ntrkpt1n->Fill(jet_ntrkpt1[i]);
                            }
                        }
                    }
                    // loose
                    ...
                    if(Cntag2) {
                        ...
                        if(otfile) {
                            ...
                            H_T4->Fill(HT);
                            npass+=1;
                            if(ntag3>2) acount->Fill(18);
                            if(ntag3>3) acount->Fill(19);
                        }
                        ...
                        std::cout<<"nJloosetagged3 (logTheta2D+Jloosetag2+Jloosetag1), nJloosetag2(logIPsig+Jloosetag1), nJloosetag1(alphaMax) "<<nloosetag3<<" "<<nloosetag2<<" "<<nloosetag1<<std::endl;
                        std::cout<<"nJtagged3 (logTheta2D+Jtag2+Jtag1), nJtag2(logIPsig+Jtag1), nJtag1(alphaMax) "<<ntag3<<" "<<ntag2<<" "<<ntag1<<std::endl;
                    }
                }
            }
        }
