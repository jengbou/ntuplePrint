...
void MergeHistsN_EXO16003(float goalintlum, float xsec, int nfiles, std::string binname, std::string ohname, bool donorm, std::string bbname)
{
    ...
    double norm;
    if(donorm) {
        HistNormN_EXO16003(norm,xsec,nfiles,binname,bbname);  // this gives the total number of events in each bin before all selections using the eventCountPreTrigger histogram
    } else{
        norm=1.;
    }
...
//         "hjptfra2","hjptfrbc","hjptfra1c","hjptfra2c","hjptb",
    ...
    for(int i=0;i<nhist;i++) {
        ...
        vv[i]=HistManN_EXO16003(goalintlum,histnames[i],norm,outnorm,xsec,nfiles,binname,donorm,bbname);
    }
    ...
    for(int i=0;i<nhist2;i++) {
        ...
        vv2[i]=HistMan2N_EXO16003(goalintlum,histnames2[i],norm,outnorm2,xsec,nfiles,binname,donorm,bbname);
    }
