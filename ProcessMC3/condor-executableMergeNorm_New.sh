...
FINAL_PREFIX_NAME=`echo ${OUT_DIR}/${UNIQUE_ID}/${CUTIDX}/logs/${CONDOR_PROCESS}`
FINAL_ROOT_OUTDIR=`echo ${OUT_DIR}/${UNIQUE_ID}/${CUTIDX}/`
FINAL_LOG=`echo ${FINAL_PREFIX_NAME}.log`

#
# run c
#
./main $OPT_MODE1 $OPT_MODE2 $RUN_MODE $BLIND $EXO16003 $FINAL_ROOT_OUTDIR $PMODE $RNGL $RNGH $FIDX > $FINAL_LOG 2>&1
